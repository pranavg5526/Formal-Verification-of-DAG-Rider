           ...
           <3>1 ASSUME NEW p \in ProcessorSet, NEW w \in WaveSet, NEW E \in SUBSET(WaveSet), UpdateWaveTn(p, w, E)
                ...
                <4>1 ASSUME NEW q \in ProcessorSet, NEW x \in WaveSet, NEW y \in leaderReachablity'[q][x].edges
                     ...
                     <5>3 CASE q # p \/ x # w
                          ...
                          <6> QED BY <6>1, <6>2, <4>1, <2>1 DEF Invariant3, StateType, CommitWithRefType, DecidedWaveType, LeaderReachabilityType, LeaderSeqType
                     <5> QED  BY <5>2, <5>3 
 ...
 <1>2 StateType /\ StateType' /\ Invariant4 /\ [Next]_vars => Invariant4'
      <2>1 ASSUME StateType, StateType', Next, Invariant4
           ...
           <3>1 ASSUME NEW p \in ProcessorSet, NEW w \in WaveSet, NEW E \in SUBSET(WaveSet), UpdateWaveTn(p, w, E)
                ...
                <4>1 ASSUME NEW q \in ProcessorSet, NEW x \in WaveSet, NEW y \in WaveSet, Contains(x, commitWithRef'[q][y])
                     ...
                     <5>1 CASE p = q
                          ...
                          <6>2 CASE x # w
                               <7>1 CASE y # w
                                    BY <7>1, <6>2, <4>1, <3>1, <2>1 DEF StateType, CommitWithRefType, DecidedWaveType, LeaderReachabilityType, LeaderSeqType, Invariant4, UpdateWaveTn
                               <7>2 CASE y = w
                                    <8>1 E # {}
                                    ...
                                    <8> QED BY <8>2, <8>1, <8>3, MaxInPlt, <4>1, <2>1, <3>1 DEF Invariant4
                               <7> QED BY <7>1, <7>2
                          <6> QED BY <6>1, <6>2
                     <5>2 CASE p # q
                          BY <5>2, <4>1, <3>1, <2>1 DEF StateType, CommitWithRefType, DecidedWaveType, LeaderReachabilityType, LeaderSeqType, Invariant4, UpdateWaveTn
                     <5> QED BY <5>1, <5>2 
           ...
           <3>2 ASSUME NEW p \in ProcessorSet, NEW w \in WaveSet, UpdateDecidedWaveTn(p, w)
                ...
                <4>1 ASSUME NEW q \in ProcessorSet, NEW x \in WaveSet, NEW y \in WaveSet, Contains(x, commitWithRef'[q][y])
                     PROVE leaderReachablity'[q][x].exists = TRUE
                     BY <4>1, <3>2, <2>1 DEF StateType, CommitWithRefType, DecidedWaveType, LeaderReachabilityType, LeaderSeqType, Invariant4, UpdateDecidedWaveTn
                <4> QED BY <4>1 DEF Invariant4
           <3> QED BY <3>1, <3>2, <2>1 DEF Next
 ...
 <1>2 StateType /\ StateType' /\ Invariant5 /\ [Next]_vars /\ Invariant4 /\ Invariant4' => Invariant5'
      <2>1 ASSUME StateType, StateType', Next, Invariant5, Invariant4, Invariant4'
           ...
           <3>1 ASSUME NEW p \in ProcessorSet, NEW w \in WaveSet, NEW E \in SUBSET(WaveSet), UpdateWaveTn(p, w, E)
                ...
                <4>1 ASSUME NEW q \in ProcessorSet, NEW x \in WaveSet, NEW y \in WaveSet, Contains(x, commitWithRef'[q][y])
                     ...
                     <5>1 CASE p = q
                          ...
                          <6>2 CASE x # w
                               <7>1 CASE y # w
                                    BY <7>1, <6>2, <4>1, <3>1, <2>1 DEF StateType, CommitWithRefType, DecidedWaveType, LeaderReachabilityType, LeaderSeqType, Invariant5, UpdateWaveTn
                               <7>2 CASE y = w
                                    ...
                                    <8>4 IsPrefix(commitWithRef'[q][x], commitWithRef[q][max(E)])
                                         BY <8>1, <8>2, <8>3, <4>1, <2>1 DEF StateType, CommitWithRefType, DecidedWaveType, LeaderReachabilityType, LeaderSeqType, Invariant5
                          ...
                          <6> QED BY <6>1, <6>2
                     <5>2 CASE p # q
                          BY <5>2, <4>1, <3>1, <2>1 DEF StateType, CommitWithRefType, DecidedWaveType, LeaderReachabilityType, LeaderSeqType, Invariant5, UpdateWaveTn
                     <5> QED BY <5>1, <5>2 
           ...
           <3>2 ASSUME NEW p \in ProcessorSet, NEW w \in WaveSet, UpdateDecidedWaveTn(p, w)
                ...
                <4>1 ASSUME NEW q \in ProcessorSet, NEW x \in WaveSet, NEW y \in WaveSet, Contains(x, commitWithRef'[q][y])
                     PROVE IsPrefix(commitWithRef'[q][x], commitWithRef'[q][y])
                     BY <4>1, <3>2, <2>1 DEF StateType, CommitWithRefType, DecidedWaveType, LeaderReachabilityType, LeaderSeqType, Invariant5, UpdateDecidedWaveTn
                <4> QED BY <4>1 DEF Invariant5
           <3> QED BY <3>1, <3>2, <2>1 DEF Next
 ...
 <1>2 StateType /\ StateType' /\ Invariant6 /\ [Next]_vars /\ Invariant3 /\ Invariant3' => Invariant6'
      <2>1 ASSUME StateType, StateType', Next, Invariant6, Invariant3, Invariant3'
                          ...
                          <6>3 leaderReachablity'[q][x].edges # {} => commitWithRef'[q][max(leaderReachablity'[q][x].edges)] = commitWithRef[q][max(leaderReachablity[q][x].edges)]
                               <7>1 leaderReachablity[q][x].edges # {} => max(leaderReachablity[q][x].edges) # w \/ q # p
                                    <8>1 leaderReachablity[q][x].edges # {} => leaderReachablity[q][max(leaderReachablity[q][x].edges)].exists = TRUE
                                         ...
                                         <9> QED BY <2>1, <4>1, <3>1, <9>2 DEF StateType, CommitWithRefType, DecidedWaveType, LeaderReachabilityType, LeaderSeqType, Invariant3
           ...
           <3>2 ASSUME NEW p \in ProcessorSet, NEW w \in WaveSet, UpdateDecidedWaveTn(p, w)
                ...
                <4>1 ASSUME NEW q \in ProcessorSet, NEW x \in WaveSet, leaderReachablity'[q][x].exists = TRUE
                     PROVE commitWithRef'[q][x] = IF leaderReachablity'[q][x].edges = {} THEN <<x>> ELSE Append(commitWithRef'[q][max(leaderReachablity'[q][x].edges)], x)
                     BY <4>1, <3>2, <2>1 DEF StateType, CommitWithRefType, DecidedWaveType, LeaderReachabilityType, LeaderSeqType, Invariant6, UpdateDecidedWaveTn
                <4> QED BY <4>1 DEF Invariant6
           <3> QED BY <3>1, <3>2, <2>1 DEF Next
      ...
      <2>1 ASSUME StateType, StateType', Next, Invariant7, Invariant6, Invariant3
           ...
           <3>1 ASSUME NEW p \in ProcessorSet, NEW w \in WaveSet, NEW E \in SUBSET(WaveSet), UpdateWaveTn(p, w, E)
                ...
                <4>1 ASSUME NEW q \in ProcessorSet, NEW r \in ProcessorSet, NEW x \in WaveSet, leaderReachablity'[q][x].exists = leaderReachablity'[r][x].exists
                     ...
                     <5>2 CASE q # r
                          <6>1 CASE x # w
                               BY <6>1, <4>1, <3>1, <2>1 DEF UpdateWaveTn, StateType, CommitWithRefType, DecidedWaveType, LeaderReachabilityType, LeaderSeqType, Invariant7
                          <6>2 CASE x = w
                               <7>1 CASE q = p
                                    ...
                                    <8>7 E # {} => leaderReachablity[r][max(E)].exists /\ leaderReachablity[q][max(E)].exists
                                         BY <8>4, MaxInPlt, <3>1, <4>1, <2>1, <7>1 DEF Invariant3, UpdateWaveTn
                                    <8>6 E # {} => commitWithRef[q][max(E)] = commitWithRef[r][max(E)]
                                         BY <4>1, <3>1, MaxInPlt, <2>1, <8>7 DEF Invariant7
                               ...
                               <7>2 CASE r = p
                                    ...
                                    <8>7 E # {} => leaderReachablity[q][max(E)].exists /\ leaderReachablity[r][max(E)].exists
                                         BY <8>4, MaxInPlt, <3>1, <4>1, <2>1, <7>2 DEF Invariant3, UpdateWaveTn
                                    <8>6 E # {} => commitWithRef[r][max(E)] = commitWithRef[q][max(E)]
                                         BY <4>1, <3>1, MaxInPlt, <2>1, <8>7 DEF Invariant7
                                    <8> QED BY <8>3, <8>4, <8>5, <8>6, <8>1     
                               <7>3 CASE q # p /\ r # p
                                    BY <7>3, <4>1, <3>1, <2>1 DEF UpdateWaveTn, StateType, CommitWithRefType, DecidedWaveType, LeaderReachabilityType, LeaderSeqType, Invariant7
           ...
           <3>2 ASSUME NEW p \in ProcessorSet, NEW w \in WaveSet, UpdateDecidedWaveTn(p, w)
                ...
                <4>1 ASSUME NEW q \in ProcessorSet, NEW r \in ProcessorSet, NEW x \in WaveSet, leaderReachablity'[q][x].exists = leaderReachablity'[r][x].exists
                     PROVE commitWithRef'[q][x] = commitWithRef'[r][x]
                     BY <4>1, <3>2, <2>1 DEF StateType, CommitWithRefType, DecidedWaveType, LeaderReachabilityType, LeaderSeqType, Invariant7, UpdateDecidedWaveTn
                <4> QED BY <4>1 DEF Invariant7
           <3> QED BY <3>1, <3>2, <2>1 DEF Next
                     ...
                     <5>2 CASE x # z
                          ...
                          <6>3 CASE p = q /\ w = z
                               ...
                               <7>7 Contains(x, commitWithRef[q][max(E)])
                                    BY <7>6, <7>5, <6>2, <7>4, <7>2, <3>1, <2>1, <4>1, MaxInPlt DEF Invariant8
                               <7> QED BY <7>7, <7>6, <7>3 DEF Contains
                          <6>4 CASE p # q \/ w # z
                               BY <6>2, <4>1, <6>4, <3>1, <2>1 DEF StateType, CommitWithRefType, DecidedWaveType, LeaderReachabilityType, LeaderSeqType, UpdateWaveTn, Invariant8
                          <6> QED BY <6>3, <6>4
                     <5> QED BY <5>1, <5>2
                <4> QED BY <4>1 DEF Invariant8
           <3>2 ASSUME NEW p \in ProcessorSet, NEW w \in WaveSet, UpdateDecidedWaveTn(p, w)
                PROVE Invariant8'
                BY <3>2, <2>1 DEF StateType, CommitWithRefType, DecidedWaveType, LeaderReachabilityType, LeaderSeqType, Invariant8, UpdateDecidedWaveTn
           <3> QED BY <3>1, <3>2, <2>1 DEF Next
                          ...
                          <6>1 CASE r = p
                               ...
                               <7>3 CASE decidedWave'[q] # x 
                                    ...
                                    <8>2 Contains(decidedWave'[q], commitWithRef[r][max(E)])
                                         ...
                                         <9> QED BY <9>1, <4>1, <9>2, <9>3, <2>1, <5>1 DEF Invariant9, StateType, CommitWithRefType, DecidedWaveType, LeaderReachabilityType, LeaderSeqType
